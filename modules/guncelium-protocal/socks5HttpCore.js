...
function buildSocks5ConnectDomainOrThrow(host, port) {
    ...
    if (!targetHost) throw new Error('targetHost must be non-empty');
...
function buildHttpGetRequestOrThrow(host) {
    ...
    if (!h) throw new Error('host must be non-empty');
...
async function socks5HttpGetWithCreateConnectionOrThrow(opts) {
    requireObject(opts, 'opts');
    requireFunction(opts.createConnection, 'opts.createConnection');
    ...
    return await new Promise((resolve, reject) => {
        ...
        const socket = opts.createConnection({ host: socksHost, port: socksPort });
        if (!socket || typeof socket.on !== 'function') return reject(new Error('createConnection must return a socket with on()'));
        if (typeof socket.write !== 'function') return reject(new Error('socket.write is required'));
        if (typeof socket.destroy !== 'function') return reject(new Error('socket.destroy is required'));
...
module.exports = {
    socks5HttpGetWithCreateConnectionOrThrow,
};
